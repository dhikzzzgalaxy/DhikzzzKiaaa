const { useState, useMemo, useEffect } = React;
            ...
            <div onClick={() => onClick(app)} className="group relative bg-white/[0.02] border border-white/[0.05] hover:border-blue-500/40 rounded-2xl p-4 cursor-pointer transition-all duration-500 overflow-hidden">
                ...
                <div className="relative z-10 flex items-start gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-blue-400/80 truncate mt-0.5 inline-block">
                            {app.developer}
                        </p>
            ...
            return (
                <div className="fade-in slide-in-from-bottom-4 duration-700 pb-24">
                    <section className="relative pt-32 pb-20 px-4 md:px-8 max-w-7xl mx-auto flex flex-col items-center text-center">
                        ...
                        <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-white/5 border border-white/10 text-blue-400 text-sm font-medium mb-6 backdrop-blur-md">
    ...
    <span className="mt-2 text-transparent bg-clip-text bg-gradient-to-r from-blue-400 via-blue-400 to-purple-500 drop-shadow-[0_0_20px_rgba(0,243,255,0.45)]">
        Dhikzzz Galaxy
    </span>
</h1>
                        ...
                        <div className="relative w-full max-w-2xl group">
                            ...
                            <div className="relative flex items-center bg-[#0a0a0f] border border-white/10 rounded-2xl px-4 py-3 backdrop-blur-xl">
                                ...
                                {searchQuery && (
                                    <button onClick={() => setSearchQuery('')} className="flex items-center justify-center w-8 h-8 rounded-full bg-white/10 hover:bg-white/20 text-slate-400 hover:text-white transition-all shrink-0">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M18 6 6 18"/><path d="m6 6 12 12"/></svg>
                                    </button>
                                )}
                                <button className="hidden sm:flex px-6 py-2 bg-gradient-to-r from-blue-500 to-purple-600 hover:from-blue-400 hover:to-purple-500 text-white font-semibold rounded-xl transition-all shadow-[0_0_15px_rgba(0,243,255,0.3)]">Search</button>
                            </div>
                        </div>
                    </section>
                                ...
                                <div className="flex gap-4 md:gap-6 overflow-x-auto pb-6 scrollbar-hide -mx-4 px-4 md:mx-0 md:px-0 snap-x">
                                    {recentApps.map(app => (
                                        <div key={app.id} className="snap-start">
                                            <RecentAppCard app={app} onClick={onAppSelect} />
                                        </div>
                                    ))}
                                </div>
                            </section>
                                ...
                                <div>
                                    ...
                                    {filteredApps.length > 10 && (
                                        <div className="mt-10 flex justify-center">
                                            <button
                                                onClick={onShowAll}
                                                className="px-8 py-4 rounded-2xl bg-white/[0.05] border border-white/[0.1] text-slate-300 font-semibold hover:bg-white/[0.1] hover:border-blue-500/30 hover:text-blue-400 transition-all flex items-center gap-3 group"
                                            >
        ...
        const AppDetailView = ({ app, onBack }) => {
            ...
            return (
                <div className="fade-in slide-in-from-right-8 duration-500 pb-24 min-h-screen">
                    ...
                    <p className="text-blue-400 font-medium mb-6 text-lg inline-block">
                        {app.developer}
                    </p>
...
<div>
    ...
    <span className="text-xs text-slate-500">
        {mirror.size}
                                ...
                                <button
    onClick={() => {
        const randomV = Math.random().toString(36).substring(2, 7);
        const shareUrl = `${window.location.origin}${window.location.pathname}?v=${randomV}&app=${app.id}`;
...
>
    <Share2 size={18} /> Bagikan Aplikasi
        ...
        function App() {
            ...
            // Dynamic URL logic to refresh appcard preview
            useEffect(() => {
                const urlParams = new URLSearchParams(window.location.search);
                if (!urlParams.has('v')) {
                    const randomStr = Math.random().toString(36).substring(2, 7);
                    urlParams.set('v', randomStr);
